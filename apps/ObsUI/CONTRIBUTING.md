...
- 不新增任意外部网络端点、自动化或可编辑 Shell 执行。允许本机只读解析 Codex 会话中的 `rate_limits`；允许 H.D.D 通过本机 `codex exec --ephemeral --json --sandbox read-only` 进行只读 SSE 回复；允许手动触发、固定 IPinfo endpoint 且强制经用户配置 loopback 代理的出口 IP 查询。不得提取、保存或上传 `auth.json`、令牌、账号信息或未授权聊天内容。
