...
`pnpm dev` 会自动打开当前界面入口 `http://127.0.0.1:5173/?ui=tab-v2`。如果浏览器没有自动打开，请手动访问这个完整地址；根地址 `/` 也会进入同一套 Tab V2 界面。
...
当前页面始终由 `TabModalV2` 提供统一的五 Tab 工作台，根地址 `/` 与 `/?ui=tab-v2` 使用同一入口。目标、本机、仓库、文献和 H.D.D 五个 Tab 共用同一套 V2 外壳与内容区域；关闭按钮回到“目标”Tab，自动化计时固定进入“目标”Tab。H.D.D 的“设置”页提供本机桥接状态和 JSON 数据备份/恢复入口。
...
代码使用 MIT License。MIT 不会自动授予第三方素材的版权许可；新增外部素材前应单独确认其可再分发权。
