...
普通新克隆已包含 ObsUI 文件，不需要递归子模块命令。旧电脑的 ObsUI 独立 `.git` 仅在旧电脑保留；迁移提交以主仓库版本为准。`pnpm dev` 和 `start-obsui.ps1` 都会打开 `http://127.0.0.1:5173/?ui=tab-v2`；根地址 `/` 也会进入同一套 Tab V2 界面。
...
- 新电脑启动后访问 **http://127.0.0.1:5173/?ui=tab-v2**，在顶部选择 H.D.D Tab、左侧选择“H.D.D 设置”，再在“本地数据”卡选择“导入 JSON 备份”，选中上述文件，确认替换新电脑当前 ObsUI 数据。不要在已有重要数据的浏览器中未经核对执行替换。
- 导入后仍使用 **http://127.0.0.1:5173/?ui=tab-v2** 的当前界面，核对“选课结束”和“电信维修,上午”两项任务。
